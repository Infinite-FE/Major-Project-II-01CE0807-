...
def train_svm(num_epochs=10):
    ...
    for epoch in range(start_epoch, num_epochs):
        try:
            ...
            checkpoint_path = os.path.join(checkpoint_dir, f"svm_epoch_{epoch + 1}.joblib")
            joblib.dump(model, checkpoint_path)
            print(f"💾 Checkpoint saved: {checkpoint_path}")

        except KeyboardInterrupt:
            print("\n⏸️ Training paused by user (Ctrl+C). Saving progress...")
            checkpoint_path = os.path.join(checkpoint_dir, f"svm_paused_epoch_{epoch + 1}.joblib")
            joblib.dump(model, checkpoint_path)
...
if __name__ == "__main__":
    train_svm(num_epochs=num_epochs)
