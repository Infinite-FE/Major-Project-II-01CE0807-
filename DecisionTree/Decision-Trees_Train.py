...
# TRAINING FUNCTION
# ===============================
def train_decision_tree(num_epochs=10):
    ...
    for epoch in range(start_epoch, num_epochs):
        try:
            ...
            # Log loss (cross-entropy equivalent)
            train_loss = log_loss(y_train, train_probs, labels=[0, 1])
            val_loss = log_loss(y_val, val_probs, labels=[0, 1])

            # Print formatted output like deep learning logs
            print(f"train Loss: {train_loss:.4f} Acc: {train_acc:.4f}")
            print(f"val Loss: {val_loss:.4f} Acc: {val_acc:.4f}")

            # Save checkpoint
            checkpoint_path = os.path.join(checkpoint_dir, f"decision_tree_epoch_{epoch + 1}.joblib")
            joblib.dump(model, checkpoint_path)
            print(f"💾 Checkpoint saved: {checkpoint_path}")

        except KeyboardInterrupt:
            print("\n⏸️ Training paused by user (Ctrl+C). Saving current progress...")
            checkpoint_path = os.path.join(checkpoint_dir, f"decision_tree_paused_epoch_{epoch + 1}.joblib")
            joblib.dump(model, checkpoint_path)
...
if __name__ == "__main__":
    train_decision_tree(num_epochs=num_epochs)